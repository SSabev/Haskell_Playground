...
prop_deterministic m xs  =  accepts m str == accepts (deterministic m) str
    where
      str  =  makeInput (alph m) xs
...
traces m  =  trace (start m)
    where
      ...
      trace q (x:xs) = [ [State q0, Symbol a] ++ t | (q0,a,q1) <- trans m, t <- trace q1 xs, x == a, q == q0 ]
